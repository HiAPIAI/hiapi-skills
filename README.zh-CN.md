...
这是 HiAPI 官方 AI Agent 技能目录，适用于 OpenClaw、Claude Code、Codex、OpenCode、Cursor 类 Agent 工作流，以及其他可以读取本地 skill 的工具。它帮助你先选对 HiAPI 入口，再安装具体的聚焦 skill。

HiAPI 是为开发者打造的 AI API 平台：一个 API，所有 AI 模型。这些 skill 把常用的图像和视频生成工作流封装好，让 AI Agent 不需要猜接口和参数，就能按明确且受限的模型路线完成任务。

当你想选择 HiAPI skill 时，先看这个仓库。准备开始生成图片或视频时，再安装对应的 skill 仓库。
...
| Agent Skills | [hiapi-skills](https://github.com/HiAPIAI/hiapi-skills) | 想浏览可用的 HiAPI skills，并选择一个聚焦、模型路由明确的工作流。 |
...
- **Agent Skills** — 给 Agent 用的聚焦工作流，每个工作流声明单一模型或固定模型路由；另有纯提示词工具把简报变成可直接生成的视频提示词。
...
| Food Commercial Video | 咖啡倾倒、饮料飞溅、食品微距、包装产品英雄镜头、餐厅氛围广告 | Kling 3.0 Omni + Seedance 2.0 Fast | [hiapi-food-commercial-video-skill](https://github.com/HiAPIAI/hiapi-food-commercial-video-skill) |
...
| 制作咖啡、饮料、食品特写、包装产品或餐厅广告镜头 | [Food Commercial Video Skill](https://github.com/HiAPIAI/hiapi-food-commercial-video-skill) |
...
如果你要一个稳定、明确、模型路由受限的工作流，用 skill 更合适。如果你希望聊天 Agent 能发现和调用更多 HiAPI 工具，用 Remote MCP 更合适。MCP 端点是 `https://mcp.hiapi.ai/mcp`。
...
| 适合 | 稳定、聚焦、模型路由明确且受限的工作流 | 聊天 Agent 在一次会话里需要发现并调用多个 HiAPI 工具 |
| 模型范围 | 一个生成 skill 对应一个模型或一组固定路由，另有纯提示词 Skill | 暴露的所有 MCP 工具：`generate_image`、`generate_video`、`list_models` 等 |
...
| 什么时候选它 | 工作流和允许的模型路由固定，要零配置漂移 | 事先不知道 Agent 会用哪个工具 |

如果两个都合适，先装聚焦 skill——运动部件最少。等需要在聊天里覆盖更多工具时再加 Remote MCP。
...
每个生成 skill 都提供配置检查。多数使用 `scripts/check-config.mjs`；Food Commercial Video 使用 `node scripts/hiapi-food-commercial-video.mjs --check`。
...
如果我要制作咖啡、饮料、食品特写、包装产品或餐厅广告镜头，安装或使用：
